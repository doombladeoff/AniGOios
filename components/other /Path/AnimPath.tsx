...
// Создаем AnimatedPath
const AnimatedPath = Animated.createAnimatedComponent(Path);

const SvgHandwriting = () => {
    // длина пути (замерь вручную или через getTotalLength в браузере)
    const pathLength = 4800;
    const progress = useSharedValue(0);

    useEffect(() => {
        progress.value = withRepeat(withTiming(1, { duration: 6000 }), -1, true); // 4 секунды
    }, []);

    // Animated props для strokeDashoffset
    ...
    return (
        ...
        <Svg height="250" width={width} viewBox={`0 0 ${width * 2.3} ${width * 1.15}`}>
            <Defs>
